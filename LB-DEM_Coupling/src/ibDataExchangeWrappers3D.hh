...
namespace plb {
  ...
  void setEllipsoidOnLattice(MultiBlockLattice3D<T,Descriptor> &lattice,
                           LiggghtsCouplingWrapper &wrapper,
                           PhysUnits3D<T> const &units,
                           bool initVelFlag)
  {
    std::vector<plint> dummyExcludeType;
    setEllipsoidOnLattice(lattice,wrapper,units,dummyExcludeType,initVelFlag);
  }
                           ...
                           bool initVelFlag)
  {
    ...
    for(plint iS=0;iS<nPart;iS++){
      ...
      SetSingleEllipsoid3D<T,Descriptor> *sss = 0;
	...
	angmom[i] = units.getLbAngmom(wrapper.lmp->atom->angmom[iS][i]);
      }
