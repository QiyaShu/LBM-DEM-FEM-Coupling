...
namespace plb {
  ...
  public:
    ...
    T getLbTorque(T const physTorque) const { return physTorque / torqueFactor; };
    T getLbAngmom(T const physAngmom) const { return physAngmom / (torqueFactor*param.getDeltaT()*t_p); };
    T getLbMass(T const physMass) const { return physMass / (rho_p*pow(l_p*param.getDeltaX(),3)); };
    ...
    T getPhysFreq(T const lbFreq) const { return lbFreq / t_p / param.getDeltaT(); };
