...
namespace plb{
  ...
  struct SetSingleEllipsoid3D : public BoxProcessingFunctional3D_L<T,Descriptor> {
  public:
    SetSingleEllipsoid3D(T *x_, T *v_, T *omega_, T *com_, T *s_, T *q_, int id_, bool initVelFlag_ = false)
      : x(x_),v(v_),omega(omega_),com(com_),s(s_),q(q_),id(id_), initVelFlag(initVelFlag_) {}
    SetSingleEllipsoid3D(const SetSingleEllipsoid3D &orig)
      : x(orig.x),v(orig.v),omega(orig.omega),com(orig.com),
        s(orig.s),q(orig.q),id(orig.id),initVelFlag(orig.initVelFlag) {}

    Box3D getBoundingBox() const { return Box3D(x[0]-s[0]-2,x[0]+s[0]+2,
                                                x[1]-s[0]-2,x[1]+s[0]+2,
                                                x[2]-s[0]-2,x[2]+s[0]+2);}

    virtual void process(Box3D domain, BlockLattice3D<T,Descriptor> &lattice);
    virtual void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
    SetSingleEllipsoid3D<T,Descriptor>* clone() const;
  private:
    T *x,*v,*omega,*com,*s,*q; // com: center of mass
    int id;
    bool initVelFlag;
    T calcSolidFractionE(T const dx_, T const dy_, T const dz_, T const sa_, T const sb_, T const sc_, T const q0_, T const q1_, T const q2_, T const q3_);
