...
int main(int argc, char* argv[]) {
	...
	//const plint logSteps = 10;
 	int ibIter = 5; // Iterations for the immersed boundary method.	5 not enough if curvature large?
 ...
 */
	...
	//TriangleSet<T> OblateMesh = TriangleSet<T>("oblateSurface.stl", (T) 1e-6);
 ...
 */
	...
	for (plint iT=0; iT<=maxSteps; ++iT) {
		...
		if(iT%vtkSteps == 0 && iT > 0) { // LIGGGHTS does not write at timestep 0
			...
			//OblateMesh.writeBinarySTL(createFileName(demOutDir+"surface_", iT, 6) + ".stl");
		...
		OblateMesh.scale(r[0],r[1],r[2]);
		OblateMesh.rotateAtOrigin(normedAxis, theta);
		OblateMesh.translate(position);
		...
		OblateMesh.scale(units.getLbLength(1),units.getLbLength(1),units.getLbLength(1));
		DEFscaledMesh<T> *MeshDef = new DEFscaledMesh<T>(OblateMesh, 0, 0, 0, Dot3D(0, 0, 0));
