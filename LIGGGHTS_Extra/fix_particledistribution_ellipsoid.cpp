...
FixParticledistributionEllipsoid::FixParticledistributionEllipsoid(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  restart_global = 1;
  ...
  templates = new FixTemplateEllipsoid*[ntemplates];
  ...
  do {
    ...
    templates[itemp] = static_cast<FixTemplateEllipsoid*>(modify->fix[ifix]);
  ...
  maxnspheres = 0;
  for(int i = 0; i < ntemplates;i++)
    if(templates[i]->number_ellipsoids() > maxnspheres)
      maxnspheres=templates[i]->number_ellipsoids();
...
FixParticledistributionEllipsoid::~FixParticledistributionEllipsoid()
{
    delete []templates;
    delete []distweight;
    delete []cumweight;
...
int FixParticledistributionEllipsoid::setmask()
{
    int mask = 0;
    return mask;
}
...
int FixParticledistributionEllipsoid::random_init_single(int ntotal)
{
    ninsert = ntotal;
    ninserted = 0;
...
Region* FixParticledistributionEllipsoid::randomize_single()
{
    if(ntemplates == 1){
...
void FixParticledistributionEllipsoid::random_init_list(int ntotal)
{
    ...
    if(n_pti_max_requested > n_pti_max)
    {
        ...
        if(pti_list) delete []pti_list;
        pti_list = new ParticleToInsertEllipsoid*[n_pti_max];
    }
...
int FixParticledistributionEllipsoid::randomize_list(int ntotal,int insert_groupbit,int exact_number)
{
    if(ntotal > n_pti_max)
    {
...
void FixParticledistributionEllipsoid::pre_insert()
{
...
int FixParticledistributionEllipsoid::insert(int n)
{
...
void FixParticledistributionEllipsoid::finalize_insertion()
{
    for(int i = 0; i < ntemplates; i++)
...
double FixParticledistributionEllipsoid::vol_expect()
{
    return volexpect;
}
...
double FixParticledistributionEllipsoid::mass_expect()
{
    return massexpect;
}
...
double FixParticledistributionEllipsoid::min_rad(int type)
{
    //get minrad
...
double FixParticledistributionEllipsoid::max_rad(int type)
{
    //get maxrad
...
int FixParticledistributionEllipsoid::max_type()
{
    return maxtype;
}
...
int FixParticledistributionEllipsoid::min_type()
{
    return mintype;
}
...
int FixParticledistributionEllipsoid::max_nspheres()
{
    return maxnspheres;
}
...
void FixParticledistributionEllipsoid::write_restart(FILE *fp)
{
  int n = 0;
  double list[1];
...
void FixParticledistributionEllipsoid::restart(char *buf)
{
  int n = 0;
