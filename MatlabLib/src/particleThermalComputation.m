...
load('matlabSetup_oblate636.mat');
%load('matlabSetup_prolate633.mat');

%nStep = 1;
%dt=1;

T = TlastStep;
%fileInput = "matlabTResult_"+nStep*dt+"s.mat";
%load(fileInput);
...
%figure(4);
%displaySurface(p,e,[4,8,13,16],T,'Temperature [K]');
%figure(2);
%displaySurface(p,e,[3,10,14,20],T,'Temperature [K]');
%figure(3);
%displaySurface(p,e,[5,7,11,18],T,'Temperature [K]');

if nStep~=0 && mod(nStep,OutStep)==0
    fileOutput = "results/mat/matlabTResult_"+nStep+".mat";
    save(fileOutput,'T');
